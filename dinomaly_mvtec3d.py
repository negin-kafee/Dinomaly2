...
from dataset import MVTec3DDataset
...
def train(item_list, args):
    ...
    for i, item in enumerate(item_list):
        ...
        train_data = MVTec3DDataset(root=train_path, transform=data_transform, gt_transform=gt_transform, phase="train",
                                    cat_3D=True)
        test_data = MVTec3DDataset(root=test_path, transform=data_transform, gt_transform=gt_transform, phase="test",
                                   cat_3D=True)
    ...
    model = DinomalyRGBD(encoder=encoder, bottleneck=bottleneck, decoder=decoder,
                         target_layers=target_layers,
                         fuse_layer_encoder=fuse_layer_encoder,
                         fuse_layer_decoder=fuse_layer_decoder,
                         add_class_token_weight=-1., norm_encoder_token=True,
                         rgb_ratio=0.5
                         )
    ...
    for epoch in range(int(np.ceil(total_iters / len(train_dataloader)))):
        ...
        loss_list = []
        for img, gt, label, img_path in train_dataloader:
            img = img.to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='../mvtec_3d')
    parser.add_argument('--save_dir', type=str, default='./saved_results')
    parser.add_argument('--save_name', type=str, default='dinomaly2_mvtec3d')
    ...
    #
    item_list = ['cable_gland', 'bagel', 'peach', 'carrot', 'dowel', 'foam', 'cookie',
                 'rope', 'potato', 'tire']
