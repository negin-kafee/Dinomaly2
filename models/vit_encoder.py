...
from tips import vision_transformer as vision_transformer_tips
...
def load(name, WEIGHTS_DIR="./backbones/weights"):
    ...
    if "tips" in name:
        model = vision_transformer_tips.__dict__[f'vit_{arch}'](patch_size=int(patchsize), img_size=448,
                                                                block_chunks=0, init_values=1,
                                                                ffn_layer='mlp',
                                                                interpolate_antialias=True,
                                                                interpolate_offset=0.)
        if arch == "base":
            ckpt_pth = download_cached_file(
                f"https://storage.googleapis.com/tips_data/v1_0/checkpoints/pytorch/tips_oss_b14_highres_distilled_vision.npz",
                WEIGHTS_DIR=WEIGHTS_DIR)
        elif arch == "small":
            ckpt_pth = download_cached_file(
                f"https://storage.googleapis.com/tips_data/v1_0/checkpoints/pytorch/tips_oss_s14_highres_distilled_vision.npz",
                WEIGHTS_DIR=WEIGHTS_DIR)
        elif arch == "large":
            ckpt_pth = download_cached_file(
                f"https://storage.googleapis.com/tips_data/v1_0/checkpoints/pytorch/tips_oss_l14_highres_distilled_vision.npz",
                WEIGHTS_DIR=WEIGHTS_DIR)
        else:
            raise ValueError("Invalid type of architecture. It must be either 'small' or 'base' or 'large.")

        state_dict = dict(np.load(ckpt_pth, allow_pickle=False))
        for key in state_dict:
            state_dict[key] = torch.tensor(state_dict[key])

    if "digpt" in name:
        if arch == 'base':
            state_dict = torch.load(f"{WEIGHTS_DIR}/D-iGPT_B_FT_1K.pth")['model']
        else:
    ...
    if "deit" in name:
        if arch == "base":
            ckpt_pth = download_cached_file(
                f"https://dl.fbaipublicfiles.com/deit/deit_base_patch16_224-b5f2ef4d.pth", WEIGHTS_DIR=WEIGHTS_DIR)
        elif arch == 'small':
            ckpt_pth = download_cached_file(
                f"https://dl.fbaipublicfiles.com/deit/deit_small_patch16_224-cd65a155.pth", WEIGHTS_DIR=WEIGHTS_DIR)
        else:
            raise ValueError("Invalid type of architecture. It must be either 'small' or 'base'.")
